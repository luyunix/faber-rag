"""使用 MarkItDown 实现 PDF 加载器。

本模块实现使用 MarkItDown 进行 PDF 解析，支持图像提取功能，
将 PDF 转换为标准化的 Markdown 格式并带图像占位符。

功能特性：
- 通过 MarkItDown 进行文本提取和 Markdown 转换
- 图像提取和存储
- 带元数据跟踪的图像占位符插入
- 图像提取失败时的优雅降级
"""
...
class PdfLoader(BaseLoader):
    """使用 MarkItDown 进行文本提取和 Markdown 转换的 PDF 加载器。
    
    此加载器：
    1. 从 PDF 提取文本并转换为 Markdown
    2. 提取图像并保存到 data/images/{doc_hash}/
    3. 以 [IMAGE: {image_id}] 格式插入图像占位符
    4. 在 Document.metadata.images 中记录图像元数据
    
    配置：
        extract_images: 启用/禁用图像提取（默认：True）
        image_storage_dir: 图像存储的基础目录（默认：data/images）
    
    优雅降级：
        如果图像提取失败，记录警告并继续仅文本解析。
    """
    ...
    def _extract_title(self, text: str) -> Optional[str]:
        ...
        # Fallback: use first non-empty line as title        for line in lines[:10]:
            line = line.strip()
    ...
    def _extract_and_process_images(
        ...
        try:
                ...
                for img_index, img_info in enumerate(image_list):
                    try:
                        ...
                        try:
                            ...
                            # 如果 not in cwd, use absolute path
            ...
            # Graceful degradation: return original text without images            return text_content, []
