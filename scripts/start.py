...
此脚本用于启动完整的 Faber RAG 系统，包括：
1. MCP HTTP Server (默认端口 8080)
2. Streamlit Dashboard (默认端口 8501)

使用方法:
    python scripts/start.py

或者指定端口:
    python scripts/start.py --mcp-port 8080 --dashboard-port 8501
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例:
  python scripts/start.py                          # 使用默认端口启动
  python scripts/start.py --mcp-port 9000         # 指定 MCP 端口
  python scripts/start.py --no-dashboard          # 只启动 MCP Server
  python scripts/start.py --no-mcp                # 只启动 Dashboard
        """
    )
    ...
    logger.info(f"📊 Dashboard: http://{args.host}:{args.dashboard_port}")
    logger.info("=" * 60)
    ...
    if args.clean_ports:
        ...
        if not args.no_dashboard:
            kill_process_on_port(args.dashboard_port)
        time.sleep(1)
    ...
    if not args.no_dashboard and not check_port_available(args.dashboard_port):
        logger.error(f"❌ Dashboard 端口 {args.dashboard_port} 已被占用，请使用 --clean-ports 或指定其他端口")
        sys.exit(1)
    ...
    try:
        ...
        if not args.no_dashboard:
            ...
            manager.start_process(dashboard_cmd, "Streamlit Dashboard")
            
            # 等待 Dashboard 启动
            time.sleep(3)
            wait_for_service(f"http://{args.host}:{args.dashboard_port}")
