...
用法:
    # 处理单个PDF文件    python scripts/ingest.py --path documents/report.pdf --collection contracts
    
    # 处理目录中的所有PDF    python scripts/ingest.py --path documents/ --collection technical_docs
...
def discover_files(path: str, extensions: List[str] = None) -> List[Path]:
    ...
    # 目录：: recursively find all matching files
    files = []
...
def main() -> int:
    ...
    # Print summary    print_summary(results, args.verbose)
    
    # Determine exit code    successful = sum(1 for r in results if r.success)
