#!/usr/bin/env python
"""Evaluation script for Faber RAG.

Runs batch evaluation against a golden test set and outputs a metrics report.

Usage:
    # 运行 with default settings (custom evaluator)    python scripts/evaluate.py

    # Specify a custom golden test set    python scripts/evaluate.py --test-set path/to/golden.json

    # Use a specific collection    python scripts/evaluate.py --collection technical_docs

    # JSON output    python scripts/evaluate.py --json
...
# 集合 UTF-8 encoding for Windows console
...
def main() -> int:
    ...
    # 创建 HybridSearch (unless --no-search)
    hybrid_search = None
